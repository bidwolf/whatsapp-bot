...
const {get_command} = require('../../utils/commands')
...
class WhatsAppInstance {
    ...
    /**
     * 
     * @param {{command_name:string,args:string[],groupId:string}} param0 
     * 
     */
    async chooseCommand({command_name, args,groupId}){
    ...
    setHandler() {
        ...
        sock?.ev.on('messages.upsert', async (receivedMessages) => {
            ...
            receivedMessages.messages.map(async (msg) => {
                ...
                if (messageType === 'conversation') {
                    webhookData['text'] = receivedMessages
        ...
        sock?.ev.on('error', async (error) => {
            logger.error(`Assumindo a responsa`,error)
        })
    }
