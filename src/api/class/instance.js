...
const { get_command, get_command_extended,get_command_mention} = require('../../utils/commands')
...
class WhatsAppInstance {
   ...
   * @param {*} message
   */
  async chooseCommand({ command_name, args, groupId,command_executor },method,message) {
    ...
    switch (command_name) {
      case 'ban':{
        const allowedMethods = ["mention","reply"]
        const groupInformation = await this.fetchGroupMetadata(groupId)
          if (!groupInformation) {
            logger.info("Group not found")
            return
          }
          const isAdmin = groupInformation.participants.find(p=>p.id === command_executor && p.admin)
          if(!isAdmin){
        ...
        if(!allowedMethods.includes(method)){
          ...
          await this.replyMessage(groupId,"Para executar esse comando você deve mencionar alguém ou responder a mensagem do alvo a ser banido.",message)
          return
        }
        const userNumber = typeof args === "string"?args:args.join(" ")
        if (userNumber) {
          const sanitizedNumber = userNumber.replace(/[^0-9]/g, '').slice(0, 13)
          const participant = groupInformation.participants.find(p => p.id === `${sanitizedNumber}@s.whatsapp.net`)
          if (!participant) {
            logger.info("Participant not found")
            await this.replyMessage(groupId,"Este número não se encontra no grupo.",message)
            return
          }
          if (participant.admin){
            logger.info("Participant not removed because it is admin")
            await this.replyMessage(groupId,"Não tenho permissão para banir administradores.",message)
            return
          }
          const result = await this.groupParticipantsUpdate(groupId, [participant.id], 'remove')
  ...
  async loadAvailableGroups() {
    try {
      ...
      logger.info(`Available groups loaded ${this.instance.availableGroups.length}`)
  ...
  async registerGroup(groupId) {
    try {
      const existentGroup = await Group.findOne({ key: this.key, groupId: groupId }).exec()
      if (existentGroup) {
  ...
  async unregisterGroup(groupId) {
    try {
      ...
      this.instance.availableGroups = this.instance.availableGroups.filter(g => g.groupId !== groupId)
  ...
  async addCommandToGroupBlockedList(commandName, groupId) {
    try {
      ...
      const group = await Group.findOne({ key: this.key, groupId: groupId }).exec()
      if (!group) {
        logger.info("Group not found")
        return
      }
      ...
      await group.save()
      const groupIndex = this.instance.availableGroups.findIndex(g => g.groupId === groupId)
  ...
  async removeCommandFromGroupBlockedList(commandName, groupId) {
    try {
      ...
      group.blockedCommands = group.blockedCommands.filter(c => c !== commandName)
      await group.save()
      const groupIndex = this.instance.availableGroups.findIndex(g => g.groupId === groupId)
      this.instance.availableGroups[groupIndex] = group
      ...
      for (const msg of receivedMessages.messages) {
        ...
        ...msg
          ...
          if(String(msg.message.extendedTextMessage?.text)?.includes('@')){
            webhookData.message.extendedTextMessage.text=String(webhookData.message.extendedTextMessage.text).split(' ')[0]
  ...
  async fetchGroupMetadata(groupId) {
    try {
      const result = await this.instance.sock?.groupMetadata(
        groupId
      )
      return result
    } catch (e) {
