...
exports.downProfile = async (req, res) => {
  const data = await global.WhatsAppInstances[req.query.key]?.DownloadProfile(
    req.query.id,
  );
  return res.status(201).json({ error: false, data: data });
};
...
exports.getStatus = async (req, res) => {
  const data = await global.WhatsAppInstances[req.query.key]?.getUserStatus(
    req.query.id,
  );
  return res.status(201).json({ error: false, data: data });
};
...
exports.blockUser = async (req, res) => {
  const data = await global.WhatsAppInstances[req.query.key]?.blockUnblock(
    req.query.id,
...
exports.updateProfilePicture = async (req, res) => {
  const data = await global.WhatsAppInstances[
    req.query.key
  ].updateProfilePicture(req.body.id, req.body.url);
  return res.status(201).json({ error: false, data: data });
};

exports.getUserOrGroupById = async (req, res) => {
  const data = await global.WhatsAppInstances[req.query.key].getUserOrGroupById(
    req.query.id,
  );
  return res.status(201).json({ error: false, data: data });
};
