...
import WhatsappExecutor from './executors/revokelink.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class RevokeLinkCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new RevokeLinkCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
