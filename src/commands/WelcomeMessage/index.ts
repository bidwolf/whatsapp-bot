...
import WhatsappExecutor from './executors/welcomemessage.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class WelcomeMessageCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new WelcomeMessageCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
