...
import WhatsappExecutor from './executors/togglebotstatus.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class ToggleBotStatusCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new ToggleBotStatusCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
