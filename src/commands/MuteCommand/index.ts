...
import WhatsappExecutor from './executors/mutecommand.executor.whatsapp';
...
class MuteCommandCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new MuteCommandCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
