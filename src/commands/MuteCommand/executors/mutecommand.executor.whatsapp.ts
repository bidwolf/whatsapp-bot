...
export default class WhatsappExecutor implements IExecutor<WhatsAppMessage> {
  async execute({ command }: WhatsAppMessage): Promise<void> {
    ...
    if (userNumber && groupMetadata) {
      const sanitizedNumber = sanitizeNumber(userNumber)
      const isUserSilenced = await this.muteUser(groupMetadata.id, getWhatsAppId(sanitizedNumber))
      if (isUserSilenced) {
        this.result = { message: SUCCESS_MESSAGES.PARTICIPANT_MUTED, status: '200' }
        return
      } else {
