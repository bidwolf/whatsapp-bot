...
import WhatsappExecutor from './executors/grouplink.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class GroupLinkCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new GroupLinkCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
