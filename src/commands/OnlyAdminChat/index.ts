...
import WhatsappExecutor from './executors/onlyadminchat.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class OnlyAdminChatCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new OnlyAdminChatCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
