...
import WhatsappExecutor from './executors/toggleshareinvite.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class ToggleShareInviteCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new ToggleShareInviteCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
