...
import WhatsappExecutor from './executors/toggleNSFW.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class ToggleNSFWCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new ToggleNSFWCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
