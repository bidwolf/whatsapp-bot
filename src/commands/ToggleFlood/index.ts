...
import WhatsappExecutor from './executors/toggleflood.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class ToggleFloodCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new ToggleFloodCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
