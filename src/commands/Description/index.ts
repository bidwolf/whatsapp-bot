...
import WhatsappExecutor from './executors/description.executor.whatsapp';
import { IValidationRunner } from '../../validators';
import ValidateMethods from '../../validators/ValidateMethods';

class DescriptionCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new DescriptionCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
