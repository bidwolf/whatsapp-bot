...
import WhatsappExecutor from './executors/rules.executor.whatsapp';
...
class RulesCommandFactory<ISocketMessage extends IMessage> implements ICommandFactory<ISocketMessage> {
...
export default (feedBackSender: IFeedbackSender, logger: Logger): ICommandFactory<WhatsAppMessage> => {
  const factory = new RulesCommandFactory<WhatsAppMessage>(feedBackSender, logger);
  return factory;
};
